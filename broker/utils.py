...
class MethodHandler:
    ...
    def __init__basic_method(self):
        ...
        def connect(*args):
            ...
            self.sender.send(0x0D, 0x0, 0x0, 0x12, 0x00, self.dtype_parser.encode(0x12, p_table))
            self._set_permutation(p_table)
        self.method_handlers[0x0B] = connect

        self.method_handlers[0x0C] = lambda *args: logger.info(f"[{args[5]}:{args[6]}] Disconnect")
        self.method_handlers[0x0D] = lambda *args: logger.info(f"[{args[5]}:{args[6]}] Connect Acknowledgement")
        ...
        def topic_publish(*args):
            ...
            self.sender.send(Method.AprrovePublishedTopic, 0x0, 0x0, DType.Byte, 0x00, self.dtype_parser.encode(DType.Byte, topic))
